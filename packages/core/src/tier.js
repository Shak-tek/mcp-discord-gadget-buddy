export function buildTierList(items) {
    ...
    const toTier = (k) => k < 0.15 * n ? "S" : k < 0.35 * n ? "A" : k < 0.6 * n ? "B" : k < 0.85 * n ? "C" : "D";
    return scored.map((x, idx) => ({ ...x, tier: toTier(idx) }));
}
function normalize(x) { return x > 0 ? Math.min(1, Math.log10(1 + x) / 2) : 0; }
function sentiment(pros, cons) { return (pros.length - cons.length) / Math.max(1, pros.length + cons.length); }
